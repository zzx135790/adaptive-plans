...
export function renderSkillBindingTableRows(value, nodeId) {
  return normalizeSkillBindings(value).map((binding) => {
    const route = binding.selected_skill
      ? `skill: ${binding.selected_skill}`
      : `Ada fallback: ${binding.ada_fallback}`;
    const alternatives = binding.alternatives
      .map((alternative) => `${alternative.skill} (${alternative.not_selected_reason})`)
      .join('; ') || 'none';
    return `| ${markdown(nodeId)} | ${binding.execution_order} | ${markdown(binding.behavior)} | ${markdown(route)} | ${markdown(binding.purpose)} | ${markdown(binding.selection_reason)} | ${markdown(alternatives)} | ${markdown(binding.override_reason ?? '-')} |`;
  });
}
