...
export const HOST_PROFILES = {
  'claude-code': {
    defaultTarget: () => path.join(os.homedir(), '.claude', 'skills', 'adaptive-planning-governance'),
  },
  agents: {
    defaultTarget: () => path.join(os.homedir(), '.agents', 'skills', 'adaptive-planning-governance'),
  },
};
...
async function sourceFiles(pluginRoot) {
  ...
  const skillRoot = path.join(root, 'skills', 'adaptive-planning-governance');
  const mapped = [];
...
export async function planHostAdapter(options = {}) {
  ...
  return {
    ...
    plugin: 'adaptive-planning-governance',
    target: targetRoot,
