...
const hookPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'optional-hooks', 'record-event.mjs');
const hooksConfigPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'optional-hooks', 'hooks.json');
...
test('optional hooks config uses the host-neutral Codex schema shape', async () => {
...
test('native Codex Stop input appends a sanitized event to an explicit valid plan root', async () => {
  ...
  const result = await runHook({
    ...
    session_id: 'session-1',
    turn_id: 'turn-1',
    tool_name: 'mcp__adaptive-planning-governance__plan_add_node',
    cwd: '/unrelated/cwd',
    transcript_path: '/private/transcript.jsonl',
    last_assistant_message: 'private response',
