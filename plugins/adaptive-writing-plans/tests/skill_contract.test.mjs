...
const skillPath = path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'SKILL.md');
...
test('skill contract maps execution progress to Codex without auto-resuming old plans', async () => {
  ...
  const execution = await fs.readFile(
    path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'references', 'execution.md'),
    'utf8',
  );
...
test('leaf execution is posture-bound, compact-safe, and subtractive before completeness', async () => {
  ...
  const compatibility = await fs.readFile(
    path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'references', 'compatibility.md'),
    'utf8',
  );
...
test('Codex entry prompts expose architecture, design, and navigable map outcomes', async () => {
  const agentPrompt = await fs.readFile(
    path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'agents', 'openai.yaml'),
    'utf8',
  );
...
test('architecture memory and design are separate canonical, re-entrant gates', async () => {
  ...
  const architecture = await fs.readFile(
    path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'references', 'architecture-memory.md'),
    'utf8',
  );
  const design = await fs.readFile(
    path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'references', 'design-gate.md'),
    'utf8',
  );
  const folder = await fs.readFile(
    path.join(pluginRoot, 'skills', 'adaptive-writing-plans', 'references', 'plan-folder-protocol.md'),
    'utf8',
  );
