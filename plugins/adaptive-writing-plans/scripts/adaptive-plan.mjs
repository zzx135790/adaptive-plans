...
const scripts = {
  ...
  'host:sync': ['sync-host-adapter.mjs', args.slice(2)],
  'completion:check': ['completion-check.mjs', args.slice(2)],
};
const key = ['architecture', 'design', 'plan', 'posture', 'host', 'completion'].includes(args[0]) ? `${args[0]}:${args[1]}` : args[0];
const selected = scripts[key];
if (!selected) {
  console.error('Usage: adaptive-plan overview|migrate|posture <assess|check|promote>|architecture <bootstrap|check|propose|apply>|design <start|update|record|revise|approve|brief>|plan <link-architecture|link-design|record-impact>|host sync|completion check');
  process.exit(2);
}
