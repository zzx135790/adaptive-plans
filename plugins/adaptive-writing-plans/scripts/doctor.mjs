...
for (const relative of [
  '.codex-plugin/plugin.json', '.mcp.json', 'skills/adaptive-writing-plans/SKILL.md',
...
try {
  ...
  check('manifest-name', manifest.name === 'adaptive-writing-plans', `name=${manifest.name ?? 'missing'}`);
...
try {
  ...
  check('mcp-root-env', ['ADAPTIVE_PLAN_ROOT', 'ADAPTIVE_PROJECT_ROOT', 'ADAPTIVE_ARCHITECTURE_ROOT'].every((name) => server?.env_vars?.includes(name)), 'startup-bound root variables declared');
} catch (error) {
  check('mcp-wiring', false, error.message);
}

try {
  const example = JSON.parse(await fs.readFile(path.join(root, 'hooks', 'events.example.json'), 'utf8'));
