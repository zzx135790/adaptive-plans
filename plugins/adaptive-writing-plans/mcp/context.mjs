...
export function validateContext(context) {
  ...
  let planRoot;
  if (plan_path) {
    if (typeof plan_path !== 'string') {
      throw Object.assign(
        new Error('context.plan_path must be a string'),
        { code: 'INVALID_CONTEXT' }
      );
    }
    ...
    const canonicalPlanPath = fs.existsSync(resolvedPlanPath)
      ? fs.realpathSync(resolvedPlanPath)
      : path.normalize(resolvedPlanPath);

    // Check for path traversal escape
    if (!canonicalPlanPath.startsWith(projectRoot + path.sep) && canonicalPlanPath !== projectRoot) {
      throw Object.assign(
...
export function validateSafePath(filePath, projectRoot) {
  ...
  // Check containment
  if (!canonicalPath.startsWith(projectRoot + path.sep) && canonicalPath !== projectRoot) {
    throw Object.assign(
