...
const planRoot = path.resolve(rootIndex >= 0 ? args[rootIndex + 1] : process.env.ADAPTIVE_PLAN_ROOT ?? process.cwd());
const projectRootIndex = args.indexOf('--project-root');
const projectRoot = path.resolve(projectRootIndex >= 0 ? args[projectRootIndex + 1] : process.env.ADAPTIVE_PROJECT_ROOT ?? process.cwd());
const architectureRootIndex = args.indexOf('--architecture-root');
const architectureRoot = path.resolve(architectureRootIndex >= 0 ? args[architectureRootIndex + 1] : process.env.ADAPTIVE_ARCHITECTURE_ROOT ?? defaultArchitectureRoot(projectRoot));
...
async function handle(request) {
  if (request.method === 'initialize') {
    return {
      ...
      serverInfo: { name: 'adaptive-writing-plans', version: '0.3.6' },
